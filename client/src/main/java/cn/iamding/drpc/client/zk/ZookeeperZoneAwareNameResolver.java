...
public class ZookeeperZoneAwareNameResolver extends NameResolver {
    ...
    public void start(Listener listener) {
        ...
        try {
            ...
            LOGGER.info("当前可用节点: {}", initialDiscovery.stream().map(ServiceDiscovery.HostandZone::toString).collect(Collectors.joining(", ")));
        ...
        try {
            serviceDiscovery.watchForUpdates(serviceName, updatedList -> {
                LOGGER.info("可用节点发生变化: {}", updatedList.stream().map(ServiceDiscovery.HostandZone::toString).collect(Collectors.joining(", ")));
